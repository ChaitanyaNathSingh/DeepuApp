...
function renderSettings() {
  ...
  $("view-settings").innerHTML = `
    ...
    <div class="settings-block">
      <h3>Start fresh</h3>
      <p class="hint">The app opens empty. Sample Chinnu data is optional, only if you tap the button below.</p>
      <div class="modal-actions" style="justify-content:flex-start">
        <button class="btn" id="btn-demo" type="button">Load sample bill</button>
...
function openEmiModal(item) {
  ...
  openModal(`
    ...
    <div class="form-grid">
      ...
      <label>Principal (₹) <input id="f-principal" type="number" step="0.01" min="0" value="${item?.principal ?? ""}" placeholder="Optional" /></label>
      <label>Interest rate (% p.a.) <input id="f-rate" type="number" step="0.01" min="0" value="${item?.interestRate ?? ""}" placeholder="e.g. 14.5" /></label>
  ...
  const refreshHint = () => {
    ...
    hint.textContent = suggested
      ? `Suggested EMI from principal + rate: ${formatINR(suggested)}. Type the bank EMI if it is different.`
      : "Interest is optional. Add principal + rate + months to see a suggested EMI.";
  };
